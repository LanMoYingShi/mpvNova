...
import app.mpvnova.player.databinding.DialogTrackBinding
...
internal class SubTrackDialog(private val player: MPVView) {
    ...
    fun refresh() {
        ...
        val activeBg = R.drawable.bg_tv_toggle_active
        val inactiveBg = R.drawable.bg_tv_toggle_inactive
        val activeText = ContextCompat.getColor(binding.root.context, R.color.tv_text)
        val dim = ContextCompat.getColor(binding.root.context, R.color.tv_text_dim)
        binding.primaryBtn.background = ContextCompat.getDrawable(
            binding.root.context,
            if (secondary) inactiveBg else activeBg
        )
        binding.secondaryBtn.background = ContextCompat.getDrawable(
            binding.root.context,
            if (secondary) activeBg else inactiveBg
        )
        binding.primaryBtn.setTextColor(if (secondary) dim else activeText)
        binding.secondaryBtn.setTextColor(if (secondary) activeText else dim)
        binding.dialogSubtitle.text = binding.root.context.getString(
            if (secondary) {
                R.string.dialog_subtitle_secondary_summary
            } else {
                R.string.dialog_subtitle_primary_summary
            }
        )
